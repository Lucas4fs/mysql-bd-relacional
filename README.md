...
<p>
    ...
    Tabela dos CLIENTES que armazena os dados dos clientes, tabela dos PRODUTOS que armazena os dados dos produtos e a tabela VENDAS que armazena os dados das vendas que foram feitas, a tabela VENDAS se relaciona com as outras duas tabelas, trazendo dados dos PRODUTOS vendidos e dos CLIENTES que compraram os PRODUTOS.<br>
...
<h3>
    ⛏️Para que Serve uma CONSTRAINT(LIMITAÇÃO) ?
</h3>

<p>
Uma constraint em um banco de dados é uma regra definida para impor limitações e restrições sobre os dados armazenados em uma tabela. As constraints ajudam a garantir que os dados inseridos em uma tabela estejam corretos e consistentes, protegendo a integridade dos dados e evitando erros e inconsistências.
</p>

<p>
Algumas das principais funções das constraints são:
</p>

<p>
Garantir a integridade referencial: uma constraint de chave estrangeira pode ser usada para garantir que os dados em uma tabela relacionada a outra tabela sejam consistentes e precisos.
</p>

<p>
Limitar valores aceitáveis: uma constraint de verificação pode ser usada para restringir os valores que podem ser inseridos em uma coluna, garantindo que apenas valores aceitáveis sejam armazenados na tabela.
</p>

<p>
Impedir duplicatas: uma constraint de chave única pode ser usada para garantir que não haja valores duplicados em uma coluna específica de uma tabela.
</p>

<p>
Impor restrições de integridade: uma constraint de chave primária pode ser usada para garantir que cada registro em uma tabela seja exclusivo e que haja uma chave primária definida para cada registro.
</p>

<p>
Em resumo, as constraints são importantes para garantir a integridade dos dados em um banco de dados, evitando erros e inconsistências que podem comprometer a qualidade dos dados e a confiabilidade das informações armazenadas.
</p>
...
<h3>
    ⛏️Importância de Nomear as Constraints que Tornam os ID's Chaves Estrangeiras
</h3>

<p>
    As constraints que tornam os ID's chaves entrangeiras ficam armazenadas dentro da tabela TABLE_CONSTRAINTS que fica dentro do banco INFORMATION_SCHEMA, este é um dos bancos criados automaticamente pelo MySQL para manter o funcionamento adequado do banco de dados. Nomear uma constraint que cria uma chave estrangeira facilita o trabalho de procurar a constraint dentro da tabela TABLE_CONSTRAINTS, pois essa tabela armazena constraints que também são criadas automaticamente pelo MySQL. Podemos inclusive fazer uma consulta dentro dessa tabela para encontrar a constraint.
</p>
...
<p>
Uma TRIGGER é um objeto do banco de dados que é usado para executar automaticamente uma ação em resposta a determinados eventos que ocorrem no banco de dados. As TRIGGERS são usadas para automatizar processos de negócios, aplicar regras de negócios e manter a integridade dos dados.
</p>

<p>
As TRIGGERS podem ser definidas para executar em resposta a diferentes eventos, como a inserção, atualização ou exclusão de dados em uma tabela. As TRIGGERS podem ser usadas para realizar várias tarefas, como:
</p>

<p>
Validar dados: as TRIGGERS podem ser usadas para validar dados antes que eles sejam inseridos ou atualizados em uma tabela, garantindo que os dados sejam precisos e completos.
</p>

<p>
Atualizar dados: as TRIGGERS podem ser usadas para atualizar automaticamente os dados em uma tabela quando ocorre um evento, como a inserção ou atualização de dados em outra tabela.
</p>

<p>
Manter a integridade dos dados: as TRIGGERS podem ser usadas para garantir que os dados em uma tabela estejam sempre consistentes e corretos, evitando que valores inválidos ou duplicados sejam inseridos.
</p>

<p>
Realizar auditorias: as TRIGGERS podem ser usadas para registrar automaticamente eventos e alterações no banco de dados para fins de auditoria e rastreabilidade.
</p>

<p>
Em resumo, as TRIGGERS são usadas para automatizar processos de negócios e manter a integridade dos dados em um banco de dados. As TRIGGERS são úteis para reduzir a necessidade de intervenção manual em processos de negócios e para garantir que os dados armazenados no banco de dados sejam precisos e confiáveis.
</p>
...
<p>
Uma PROCEDURE (também conhecida como PROC ou procedimento armazenado) é um objeto de banco de dados que contém um conjunto de instruções SQL que são agrupadas em uma única unidade lógica de trabalho. Ela é uma função que pode ser invocada por um programa ou script para realizar uma série de tarefas predefinidas no banco de dados.
</p>

<p>
As PROCEDURES são úteis para uma variedade de tarefas, como:
</p>

<p>
Reutilização de código: Ao escrever as instruções SQL em uma PROCEDURE, essas instruções podem ser reutilizadas em vários pontos do sistema de banco de dados, reduzindo a necessidade de duplicar o código.
</p>

<p>
Controle de acesso: As PROCEDURES permitem que um administrador de banco de dados conceda e revogue privilégios de acesso a uma determinada PROCEDURE, em vez de conceder privilégios de acesso às tabelas individuais do banco de dados.
</p>

<p>
Melhor desempenho: Ao invocar uma PROCEDURE, o banco de dados pode armazenar a PROCEDURE em cache, permitindo que ela seja executada mais rapidamente em comparação com a execução de um conjunto de instruções SQL individuais.
</p>

<p>
Implementação de regras de negócios: As PROCEDURES permitem que as regras de negócios sejam implementadas diretamente no banco de dados, garantindo que os dados estejam sempre de acordo com as políticas e regras da empresa.
</p>

<p>
Em resumo, uma PROCEDURE é uma maneira de agrupar um conjunto de instruções SQL em uma unidade lógica de trabalho que pode ser invocada por um programa ou script. As PROCEDURES são úteis para melhorar o desempenho do banco de dados, implementar regras de negócios e controlar o acesso aos dados.
</p>
...
<p>
Uma VIEW (visão ou visão virtual) é uma tabela virtual em um banco de dados relacional que não é armazenada fisicamente. Ela é uma consulta predefinida que retorna um conjunto específico de dados selecionados a partir de uma ou mais tabelas do banco de dados.
</p>

<p>
As VIEWS têm várias finalidades e benefícios, incluindo:
</p>

<p>
Simplificação das consultas: As VIEWS podem ser usadas para agrupar e simplificar as consultas complexas que envolvem várias tabelas do banco de dados. Em vez de escrever consultas longas e complexas, é possível criar VIEWS que retornem apenas as informações relevantes.
</p>

<p>
Segurança de dados: As VIEWS podem ser usadas para restringir o acesso a dados confidenciais em uma tabela do banco de dados. Por exemplo, uma VIEW pode ser criada para mostrar apenas as informações que os usuários têm permissão para acessar, ocultando os dados confidenciais.
</p>

<p>
Facilidade de manutenção: As VIEWS podem ser atualizadas facilmente sem afetar as tabelas subjacentes do banco de dados. Por exemplo, se uma tabela é renomeada ou suas colunas são alteradas, a VIEW pode ser atualizada para refletir essas alterações sem alterar a consulta original.
</p>

<p>
Desempenho: As VIEWS podem melhorar o desempenho do banco de dados, pois os dados são armazenados em cache e a consulta é pré-compilada. Isso significa que a VIEW pode ser acessada mais rapidamente do que uma consulta que precisa ser executada cada vez que é solicitada.
</p>

<p>
Em resumo, as VIEWS são consultas predefinidas que retornam um conjunto específico de dados selecionados a partir de uma ou mais tabelas do banco de dados. Elas têm várias finalidades, incluindo simplificar as consultas, aumentar a segurança de dados, facilitar a manutenção e melhorar o desempenho do banco de dados.
</p>

<h3>
    Criando da VIEW(VISUALIZAÇÃO) REL_VENDAS queque Mostra qual Cliente Comprou qual Produto e qual Foi o Valor da Venda
</h3>
...
<p>
    <a href="imagensedocumentos\83-EMPRESA.CALC_REL_VENDAS.csv" download></a>
</p>
...
<p>
    <a href="imagensedocumentos\89-EMPRESA.RANKING_PROD_MAIS_VENDIDO.csv" download></a>
</p>
...
<p>
    <img src = <a href="imagensedocumentos\95-EMPRESA.RANKING_CLIENTES_QUE_MAIS_COMPRARAM.csv" download></a>
</p>
...
<p>
    <a href="imagensedocumentos\101-EMPRESA.QTD_CLIENTE_POR_REGIAO.csv" download></a>
</p>
...
<p>
    <a href="imagensedocumentos\107-EMPRESA.PRODUTOS_PRECO_ABAIXO_IGUAL_MEDIA.csv" download></a>
</p>
...
<p>
    <a href="imagensedocumentos\113-EMPRESA.PROD_PRECO_ACIMA_MEDIA.csv" download></a>
</p>
...
<p>
    <a href="imagensedocumentos\125-EMPRESA.CLIENTES_QUE_COMPRARAM_PROD_PRECO_ABAIXO_IGUAL_MD.csv" download></a>
</p>
